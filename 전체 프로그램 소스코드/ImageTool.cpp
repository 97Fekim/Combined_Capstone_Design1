...
BEGIN_MESSAGE_MAP(CImageToolApp, CWinAppEx)
	...
	ON_COMMAND(ID_FAST_INSPECT, &CImageToolApp::OnFastInspect)
	ON_COMMAND(ID_FAST_INSPECT_EXIT, &CImageToolApp::OnFastInspectExit)
END_MESSAGE_MAP()
	...
	:m_pNewDib(NULL)
{
	...
	m_x1 = 0; m_y1 = 0; m_y1 = 0; m_x2 = 0;
	m_ROIMode = FALSE;
	m_ratio = 0;
	m_fastInspectMode = FALSE;
	// 다시 시작 관리자 지원
...
void CImageToolApp::OnFastInspect()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	CFastInspectionDlg dlg;
	if (dlg.DoModal() == IDOK)
	{
		m_fastInspectMode = TRUE;
		m_ratio = dlg.m_Ratio;
		AfxMessageBox(_T("빠른 검사를 시작합니다.\n영상 불러오기시 검사 결과가 출력됩니다."));
	}
}

void CImageToolApp::OnFastInspectExit()
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	m_fastInspectMode = FALSE;
	AfxMessageBox(_T("빠른 검사를 종료합니다"));
}
